...
class TradingConfig:
    ...
    pairs: List[str] = field(default_factory=lambda: [
        # Major caps (lower volatility)
        "BTC/AUD", "ETH/AUD",
        # Mid-tier volatility
        "SOL/AUD", "LINK/AUD", "DOT/AUD", "AVAX/AUD", "ADA/AUD", "ATOM/AUD", "XRP/AUD"
    ])
...
class CostOptimizationConfig:
    ...
    enable_decision_cache: bool = False  # Requires Redis
...
class AggressiveRiskConfig:
    ...
    pair_stop_losses: Dict[str, float] = field(default_factory=lambda: {
        "BTC/AUD": 0.03,   # 3% - lower volatility
        "ETH/AUD": 0.035,  # 3.5%
        "SOL/AUD": 0.04,   # 4% - higher volatility
        "DOGE/AUD": 0.05,  # 5% - very volatile
        "AVAX/AUD": 0.04,  # 4%
        "ARB/AUD": 0.045,  # 4.5%
    })
...
class Settings:
    ...
    def load(cls, stage: Stage = None) -> "Settings":
        ...
        # Cost optimization config from environment
        cost_opt = CostOptimizationConfig(
            enable_decision_cache=os.getenv("ENABLE_DECISION_CACHE", "false").lower() == "true"
        )
