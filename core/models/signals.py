...
class MarketIntel:
    ...
    def to_summary(self) -> str:
        ...
        signal_summaries = " | ".join([
            f"{s.source}: {s.direction:+.2f} ({s.confidence:.0%}) {s.reasoning[:80]}"
            for s in self.signals
        ])

        return (
            f"{self.pair} regime={self.regime.value} dir={self.fused_direction:+.2f} "
            f"conf={self.fused_confidence:.0%} disagree={self.disagreement:.0%}\n"
            f"Signals: {signal_summaries}"
        )
