...
class Orchestrator:
    ...
    def __init__(
        ...
        self._cycle_history = []  # Store recent cycle activity for dashboard (max 20)
        self._current_cycle_decisions = []  # Decisions being made in current cycle
    ...
    async def run_cycle(self) -> dict:
        ...
        self._current_cycle_decisions = []  # Reset for new cycle
        ...
        # Store cycle activity in history
        cycle_activity = {
            "cycle_id": self._cycle_count,
            "timestamp": cycle_start.isoformat(),
            "status": "error" if results["errors"] else "completed",
            "pairs_analyzed": results["pairs_analyzed"],
            "trades_executed": results["trades_executed"],
            "decisions": self._current_cycle_decisions.copy(),
            "portfolio_value": portfolio.total_value if 'portfolio' in dir() else None,
            "errors": results["errors"]
        }
        self._cycle_history.insert(0, cycle_activity)
    ...
    async def _process_pairs_batch(
        ...
        # Capture decisions for AI activity feed
        rejection_reasons = {s.pair: s.rejection_reason for s in validated_plan.rejected_signals} if validated_plan.rejected_signals else {}
        for intel in intel_list:
            pair = intel.pair
            result = results.get(pair, {"executed": False, "action": "HOLD"})
            decision = {
                "pair": pair,
                "action": result.get("action", "HOLD"),
                "confidence": intel.fused_confidence,
                "reasoning": self._build_reasoning(intel.signals),
                "executed": result.get("executed", False),
                "analysts": {
                    s.source: {"direction": round(s.direction, 2), "confidence": round(s.confidence, 2)}
                    for s in intel.signals
                }
            }
            if pair in rejection_reasons:
                decision["rejection_reason"] = rejection_reasons[pair]
            self._current_cycle_decisions.append(decision)
    ...
    async def _process_pair(self, pair: str, portfolio: Portfolio) -> dict:
        ...
        # Capture decision for AI activity feed
        decision = {
            "pair": pair,
            "action": result["action"],
            "confidence": intel.fused_confidence,
            "reasoning": plan.reasoning if hasattr(plan, 'reasoning') and plan.reasoning else self._build_reasoning(signals),
        ...
        if validated_plan.rejected_signals:
            decision["rejection_reason"] = validated_plan.rejected_signals[0].rejection_reason
        self._current_cycle_decisions.append(decision)

        return result

    def _build_reasoning(self, signals) -> str:
        """Build reasoning text from analyst signals"""
        parts = []
        for s in signals:
            direction_word = "bullish" if s.direction > 0.1 else "bearish" if s.direction < -0.1 else "neutral"
            parts.append(f"{s.source}: {direction_word} ({s.confidence:.0%})")
        return ", ".join(parts) if parts else "No signals"
    
    def get_cycle_activity(self, limit: int = 10) -> list:
        """Get recent cycle activity for AI transparency dashboard"""
